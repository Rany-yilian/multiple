package com.im.tools.config;


import com.im.tools.Utils;
import com.im.tools.config.CC.mp.net.public_ip_mapping;
...
public final class ConfigTools {
    ...
    public static int getHeartbeat(int min,int max){
        return Math.max(CC.mp.core.min_heartbeat, Math.min(max,CC.mp.core.max_heartbeat));
    }
